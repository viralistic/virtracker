...
# ---------------------------------------------------------------------------
# Sheets access via service account (auth lives in gauth.py)
# ---------------------------------------------------------------------------
...
def fetch_rows_via_service_account(key_path: str, sheet_id: str, tab: str):
    ...
    sa = load_sa(key_path)
    token = get_access_token(sa, SHEETS_SCOPE)
...
def embed(data, n_flags):
    ...
    # preserve any Search Console enrichment (added by enrich_gsc.py) across refreshes
    m = re.search(r'<script id="seo-data" type="application/json">(.*?)</script>', html, flags=re.S)
    if m:
        try:
            prev = json.loads(m.group(1))
            if isinstance(prev, dict) and 'gsc' in prev:
                data['gsc'] = prev['gsc']
    ...
    print(f'Embedded {n_flags} flags across {len(data["dates"])} days '
          ...
          + (' (kept existing Search Console data)' if 'gsc' in data else ''))
